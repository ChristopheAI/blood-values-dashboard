                ...
                @forelse ($overview['trendChanges'] as $change)
                    <div class="rounded-lg border border-neutral-200 p-3 text-sm dark:border-neutral-700">
                        ...
                        <div class="text-neutral-700 dark:text-neutral-300">
                            ...
                            {{ \App\Support\Format::biomarkerValue($change['previousResult']->value, $change['previousResult']->source_snippet, $change['previousResult']->value_comparator) }} {{ $change['previousResult']->unit }}
                            ->
                            {{ $change['result']->bloodTest->test_date ? \App\Support\Format::dutchDate($change['result']->bloodTest->test_date) : __('Geen datum') }}
                            {{ \App\Support\Format::biomarkerValue($change['result']->value, $change['result']->source_snippet, $change['result']->value_comparator) }} {{ $change['result']->unit }}
                            ({{ $change['changeLabel'] }})
                        </div>
                    </div>
                @empty
